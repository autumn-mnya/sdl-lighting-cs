...
{
    pauseDLL = LoadLibraryA("pause.dll");
    if (pauseDLL == nullptr) {
...
void RegisterElementSDL(std::vector<void (*)()>& handlers, const char* functionName, void (*handler)())
{
    if (pauseDLL != nullptr) {
...
{
    RegisterElementSDL(saveconfigElementHandlers, "RegisterSaveConfigElement", reinterpret_cast<void (*)()>(handler));
}